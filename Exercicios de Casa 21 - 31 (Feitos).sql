...
select
ShipCountry, year(OrderDate) as Orderdate, AverageFreight = avg (freight)
from Orders
...
Select Top 3
   OrderID ,ShipCountry, AverageFreight = avg(freight)
From Orders
...
-- ORDER ID 11051 e 11076 nao tem SHIPPEDDATE portanto nao sairam ainda ??
...
Select top 3
ShipCountry, cast(OrderDate as date) as OrderDate, AverageFreight = avg(freight)
From Orders
where OrderDate between '2015-05-06' and '2016-05-06'
group by OrderDate, ShipCountry
order by AverageFreight desc
...
select Employees.EmployeeID, OrderID, CustomerID, FirstName, LastName
from Employees
right join Orders
on Employees.EmployeeID = Orders.EmployeeID
